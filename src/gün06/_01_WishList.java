...
public class _01_WishList extends BaseDriver {
    ...
    public void addToWishList() {
        ...
        String  selectedProductName=productNameList.get(rndSayi).getText();//4 üründen birini random seç

        List<WebElement> wishButtons=driver.findElements(By.xpath("//*[@data-original-title='Add to Wish List']"));
        wishButtons.get(rndSayi).click();//random birini seç sepete tıklat

        WebElement wishListLink=driver.findElement(By.xpath("//a[@id='wishlist-total']"));
        wishListLink.click();//sepete ekle listesi

        // Wish List ekranda , ekrandaki ürünlerin listesini alalım
        List<WebElement> wishNameList=driver.findElements(By.cssSelector("[class='text-left'] a"));//sepete ekle listesi
